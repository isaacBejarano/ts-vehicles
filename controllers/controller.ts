...
// Regexp
const regexPlate = new RegExp(/^[0-9]{4}[a-zA-Z]{3}$/); // (pre) "Plate" must have 4 digits followed by 3 letters
...
function addWheelsToCurrentCar(e: Event): void {
  ...
  // toString -> <ul><li>
  for (let i = 0; i < carInfoUlLiSpans.length; i++) {
    const brandToString = cars[cars.length - 1].wheels[i].brand
      ? cars[cars.length - 1].wheels[i].brand + " /"
      : "No brand specified /";

    const diameterToString =
      cars[cars.length - 1].wheels[i].diameter > 0
